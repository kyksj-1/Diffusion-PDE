...
| `[planned]` `\rhotphys` | $\rho_t$ | PDE 物理时间 $t$ 处的分布（initial 分布的 PDE-flow push-forward） | §6 / §4 整合 |
...
| `[planned]` `\physsol` | $\mathbf{u}^\star$ | Kruzhkov 熵解（在固定 $T_{\mathrm{phys}}$） | §6 / 附录 A1 |
| `[planned]` `\physsolvis` | $\mathbf{u}^\nu$ | 带粘性 $\physvis$ 的 PDE 解 | 附录 A1 |
| `[planned]` `\initdat` | $\mathbf{u}_0$ | PDE 初值 | §6（Thm 1）/ 附录 |
...
| `[planned]` `\dShock` | $d_\Sigma(u, \tau)$ | 到 $\Sigma_{\mathrm{score}}(\tau)$ 的符号距离 | 附录 A1 |
...
| LaTeX 宏 | 数学呈现 | 含义 | 作用域 |
|---|---|---|---|
| `[planned]` `\phisbg` | $\phi^{\mathrm{sm}}_\theta$ | 平滑背景势函数 | §5 / §6 / 附录 |
| `[planned]` `\phissh` | $\phi^{\mathrm{sh}}_\theta$ | shock 符号距离场（zero on shock） | §5 / §6 / 附录 |
| `[planned]` `\jumpamp` | $\kappa_\theta$ | 局部跳跃强度（R–H 约束） | §5 / §6 / 附录 |
...
| `[planned]` `\uhat` | $\hat u(\tau)$ | 网络 score 驱动的反向 ODE 轨迹 | 附录 A1 |
| `[planned]` `\uth` | $u^\theta = \hat u(0)$ | 最终生成样本（$\tau = 0$ 时刻） | §6 / 附录 |
| `[planned]` `\muth` | $\mu_\theta = \Law(u^\theta)$ | 生成分布 | §6 / 附录 |
...
% — block I（BV-aware 参数化）—
\newcommand{\phisbg}{\phi^{\mathrm{sm}}_{\theta}}
\newcommand{\phissh}{\phi^{\mathrm{sh}}_{\theta}}
\newcommand{\jumpamp}{\kappa_{\theta}}

% — block J（轨迹）—
\newcommand{\unat}{u^{\natural}}
\newcommand{\uhat}{\hat{u}}
\newcommand{\uth}{u^{\theta}}
